...
Configured in `api/src/agent.py` and `api/scripts/embed_fpf_posts.py`:
...
| File | Changes |
|------|---------|
| `api/pyproject.toml` | Added `pgvector>=0.3.0` |
| `api/src/database.py` | Added pgvector extension init |
| `api/src/models.py` | Added embedding columns to FPFPost |
| `api/src/agent.py` | Added `search_fpf_posts` tool + embedder |
| `api/scripts/embed_fpf_posts.py` | **NEW** - Embedding generation |
| `api/scripts/run_fpf_pipeline.py` | **NEW** - Pipeline orchestration |
