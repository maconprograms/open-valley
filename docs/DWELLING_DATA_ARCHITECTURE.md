...
CREATE TABLE dwellings (
    ...
    tax_classification VARCHAR(30),     -- 'HOMESTEAD', 'NHS_RESIDENTIAL', 'NHS_NONRESIDENTIAL'
    use_type VARCHAR(50),               -- 'owner_occupied_primary', 'short_term_rental', etc.

    -- Linkage
    str_listing_id UUID REFERENCES str_listings(id),
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.tax_classification IS DISTINCT FROM NEW.tax_classification THEN
            INSERT INTO dwelling_changes (dwelling_id, change_type, field_name, old_value, new_value, changed_by)
            VALUES (NEW.id, 'update', 'tax_classification', OLD.tax_classification, NEW.tax_classification,
...
with Session(engine) as session:
    ...
    adu = Dwelling(
        ...
        dwelling_type="adu",
        tax_classification="NHS_NONRESIDENTIAL",  # Long-term rental
...
-- Track homestead conversions over time
SELECT
    DATE_TRUNC('month', dc.changed_at) as month,
    SUM(CASE WHEN dc.new_value = 'HOMESTEAD' THEN 1 ELSE 0 END) as became_homestead,
    SUM(CASE WHEN dc.old_value = 'HOMESTEAD' THEN 1 ELSE 0 END) as left_homestead
FROM dwelling_changes dc
WHERE dc.field_name = 'tax_classification'
GROUP BY 1
ORDER BY 1;
```
...
|----------|-----------|-------------------|-------------|
| 488 Phillips | CAT=R2, DESCPROP="& DWL" | None (use DESCPROP) | 1 dwelling, HOMESTEAD |
| 448 Tremblay | CAT=R2, DESCPROP="& DWL" | None (use DESCPROP) | 1 dwelling, NHS_RESIDENTIAL |
| 200 Fabio | CAT=R1, DESCPROP="& DWL" | Add ADU manually | 2 dwellings (main + ADU) |
